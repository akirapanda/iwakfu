...
item.item_set_stats.each do |stat|
  if stat.cate != "bonus"
    next
  end
  ...
  @detail = ItemSetDetail.where("item_set_id = ? and piece =?",item.id,stat.piece)[0]
  
  if @detail.nil?
    @detail = ItemSetDetail.new
    @detail.item_set_id = item.id
    @detail.piece = stat.piece
  end
